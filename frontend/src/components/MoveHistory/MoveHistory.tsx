...
export default function MoveHistory({ moves, moveIndex, onJumpToPly, moveMeta }: Props) {
  ...
  return (
    <div className="border border-gray-300 rounded-md bg-white">
      <div className="px-3 py-2 text-sm font-semibold text-zinc-800 border-b border-zinc-200">
        Move history
      </div>
      <div className="max-h-80 overflow-auto px-3 py-2 text-sm">
        {rows.length === 0 ? (
          <div className="text-zinc-600">No moves loaded.</div>
        ) : (
          <div className="space-y-1">
            {rows.map((r) => (
              <div key={r.moveNo} className="flex items-start gap-2">
                <div className="w-7 text-zinc-500 select-none">{r.moveNo}.</div>
                <button
                  type="button"
                  title={
                  ...
                  disabled={!r.white}
                  className={[
                    "px-2 py-0.5 rounded-md text-left text-zinc-600",
